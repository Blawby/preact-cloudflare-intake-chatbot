...
export class BusinessLogicHandler {
  ...
  static async handleConversation(
    ...
    if (conversationText && conversationText.trim().length >= 20) {
      try {
        ...
        context = {
          ...
          isGeneralInquiry: true, // Assume general inquiry if extraction fails
          shouldCreateMatter: false,
          state: ConversationState.GATHERING_INFORMATION // Default state
        };
      }
    } else {
    ...
    Logger.debug('🔍 Business Logic Analysis:', {
      ...
      isGeneralInquiry: await ConversationStateMachine.isGeneralInquiry(conversationText, env)
    });
  ...
  static async shouldBypassAI(conversationText: string, env?: any): Promise<boolean> {
    ...
    return state === ConversationState.READY_TO_CREATE_MATTER || 
           !ConversationStateMachine.shouldUseAIResponse();
  }
  ...
  static getSystemPromptForAI(state: ConversationState, context: ConversationContext): string {
    const basePrompt = `You are a legal intake specialist for North Carolina Legal Services. Your primary goal is to empathetically assist users, understand their legal needs, and gather necessary information to create a legal matter.
