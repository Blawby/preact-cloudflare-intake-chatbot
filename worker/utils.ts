...
export async function createMatterRecord(
  ...
  try {
    ...
    const matterId = organizationId ? organizationId + '-' + Date.now() : Date.now().toString();
    const year = new Date().getFullYear();
    const matterNumberResult = await env.DB.prepare(`
      SELECT COUNT(*) as count FROM matters 
      WHERE organization_id = ? AND strftime('%Y', created_at) = ?
    `).bind(organizationId, year.toString()).first();
    const count = (matterNumberResult as { count?: number })?.count || 0;
    const matterNumber = `MAT-${year}-${(count + 1).toString().padStart(3, '0')}`;
    await env.DB.prepare(`
      INSERT INTO matters (
        id, organization_id, client_name, matter_type, title, description, status, priority, 
