...
export async function logChatMessage(
  ...
  try {
    // const messageId = crypto.randomUUID(); // REMOVE THIS LINE
    // Instead, expect messageId to be passed in or generated by DB. Example:
    const messageId = teamId ? teamId + '-' + Date.now() : Date.now().toString();
...
export async function storeMatterQuestion(
  ...
  try {
    // const questionId = crypto.randomUUID(); // REMOVE THIS LINE
    // Instead, expect questionId to be passed in or generated by DB. Example:
    const questionId = teamId ? teamId + '-' + Date.now() : Date.now().toString();
    await env.DB.prepare(`
...
export async function storeAISummary(
  ...
  try {
    // const summaryId = crypto.randomUUID(); // REMOVE THIS LINE
    // Instead, expect summaryId to be passed in or generated by DB. Example:
    const summaryId = teamId ? teamId + '-' + Date.now() : Date.now().toString();
    await env.DB.prepare(`
